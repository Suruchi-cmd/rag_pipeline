...
class RetrieveResponse(BaseModel):
    """Response for retrieval-only endpoint (no LLM generation)."""

    source_documents: list[SourceDocument] = Field(
        default_factory=list,
        description="List of retrieved source documents ranked by similarity.",
    )
...
class ResyncRequest(BaseModel):
    """Request body for /rag/resync."""

    force_download: bool = Field(
        default=True,
        description="Force re-download the Google Sheet even if unchanged.",
    )


class ResyncResponse(BaseModel):
    """Response from /rag/resync."""

    parser_ok: bool = Field(..., description="Whether the sheet parser succeeded")
